    ...
    static class DataMapper
    {
        ...
        public static List<Post> LoadPosts()
        {
            ...
            foreach (var line in dataLines)
            {
                ...
                var authorId = int.Parse(args[5]);

                var replayIds = args[4].Split(',', StringSplitOptions.RemoveEmptyEntries)
                    .Select(int.Parse).ToArray();
                Post post = new Post(id, title, content, categoryId, authorId, replayIds);
                posts.Add(post);
            }
            return posts;
        }
