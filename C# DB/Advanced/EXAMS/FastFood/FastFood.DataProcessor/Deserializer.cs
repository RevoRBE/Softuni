...
namespace FastFood.DataProcessor
{
    public static class Deserializer
    {
        ...
        public static string ImportOrders(FastFoodDbContext context, string xmlString)
        {
            ...
            var deserializaedOrders = (OrderDto[])serializer.Deserialize(new StringReader(xmlString));

            //List<Order> orders = new List<Order>();
            List<OrderItem> orderItems = new List<OrderItem>();

            foreach (var orderDto in deserializaedOrders)
            {
                if (!IsValid(orderDto))
                {
                    sb.AppendLine(FailureMessage);
        ...
        private static bool AreValidItems(FastFoodDbContext context, OrderItems[] orderItems)
        {
